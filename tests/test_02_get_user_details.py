"""
Tests for the second scenario - 'Get extracted user details'
API endpoint: GET /api/users/3
...
class TestGetUserDetails:
    ...
    def test_user_id_response_code_200(self, response_user_id):
        assert response_user_id.status_code == 200

    def test_user_id_json_schema(self, response_user_id):
    ...
    def test_names_are_correct(self, response_user_id):
        body = response_user_id.json()
        user = body.get("data")
        assert user.get("first_name") == "Emma"
        assert user.get("last_name") == "Wong"

    def test_text_contains_word_caddy(self, response_user_id):
        body = response_user_id.json()
